...
int carmen_feof(carmen_FILE *fp)
{
  ...
  if(!fp->compressed)
    return feof(fp->fp);
      ...
      return gzeof((gzFile) fp->comp_fp);
#else
  return feof(fp->fp);
#endif
}
...
int carmen_fseek(carmen_FILE *fp, off_t offset, int whence)
{
  ...
  if(!fp->compressed)
    return fseeko(fp->fp, offset, whence);
      ...
      return gzseek((gzFile) fp->comp_fp, offset, whence);
#else
  return fseeko(fp->fp, offset, whence);
#endif
}
...
off_t carmen_ftell(carmen_FILE *fp)
{
  ...
  if(!fp->compressed)
    ...
    return gztell((gzFile) fp->comp_fp);
#else
  return ftello(fp->fp);
#endif
}
...
int carmen_fclose(carmen_FILE *fp)
{
  ...
  if(!fp->compressed)
    ...
    return gzclose((gzFile) fp->comp_fp);
#else
  return fclose(fp->fp);
#endif
}
...
size_t carmen_fread(void *ptr, size_t size, size_t nmemb, carmen_FILE *fp)
{
  ...
  if(!fp->compressed)
    ...
    return gzread((gzFile) fp->comp_fp, ptr, size * nmemb) / size;
#else
  return fread(ptr, size, nmemb, fp->fp);
#endif
}
		     ...
		     carmen_FILE *fp)
{
  ...
  if(!fp->compressed)
    ...
    return gzwrite((gzFile) fp->comp_fp, (void *)ptr, size * nmemb) / size;
#else
  return fwrite(ptr, size, nmemb, fp->fp);
#endif
}
...
char *carmen_fgets(char *s, int size, carmen_FILE *fp)
{
  ...
  if(!fp->compressed)
    ...
    return gzgets((gzFile) fp->comp_fp, s, size);
#else
  return fgets(s, size, fp->fp);
#endif
}
...
int carmen_fputc(int c, carmen_FILE *fp)
{
  ...
  if(!fp->compressed)
    ...
    return gzputc((gzFile) fp->comp_fp, c);
#else
  return fputc(c, fp->fp);
#endif
}
