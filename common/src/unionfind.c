...
static inline struct unionfind_entry *
unionfind_find_rec(unionfind_t *uf, struct unionfind_entry *a)
{
...
void unionfind_union(unionfind_t *uf, int64_t id_a, int64_t id_b)
{
    ...
    if (aroot->rank > broot->rank) {
        ...
        //aroot->representative = bot_min64(aroot->representative, broot->representative);
	aroot->representative = bot_min(aroot->representative, broot->representative);
    } else if (aroot->rank < broot->rank) {
        aroot->parent = broot->id;
        broot->representative = bot_min(aroot->representative, broot->representative);
    } else {
        broot->parent = aroot->id;
        aroot->rank++;
        aroot->representative = bot_min(aroot->representative, broot->representative);
